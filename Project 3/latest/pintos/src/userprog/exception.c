...
page_fault (struct intr_frame *f) 
{
  ...
  if (not_present && is_user_vaddr(fault_addr))
  {
	 ...
	 //lock_release (&file_lock);
       ...
       {
	 success = swap_in_from_disk (spte);
         spte->pinned = false;
       }
     }
    else
    {
      if(((PHYS_BASE - MAX_STACK_SIZE) < (uint32_t *)fault_addr)
	   && fault_addr >= (esp - 32))
      {
        /* Grow the stack. */
	success = grow_stack_by_page (vaddr);
      }
    }
  }
  /* End of Project 3 */
...
bool
grow_stack_by_page (uint32_t *vaddr)
{
  struct sup_page *spte;
  struct thread *cur = running_thread ();
  uint32_t  *paddr = palloc_get_frame (PAL_USER);

  if (!install_frame (vaddr, paddr, true, NULL, 0, DATA, PGSIZE, IN_MEMORY))
    return false;
...
bool
load_from_file (struct sup_page *spte)
{
  bool lock_held = false;

  if (exception_lock.holder != thread_current ())
  {
  ...
  //lock_acquire (&file_lock);
  if (file_read_at (spte->file, kpage, page_read_bytes, spte->ofs) 
       != (int) page_read_bytes)
  {
    free_frame (kpage);
    //lock_release (&file_lock);
  ...
  if (!install_frame (spte->vaddr, kpage, writable, spte->file, 
                      spte->ofs, spte->ptype, spte->read_bytes, IN_MEMORY))
  {
    free_frame (kpage);
    if (lock_held)
      lock_release (&exception_lock);
...
bool
swap_in_from_disk (struct sup_page *spte)
{
  bool lock_held = false;
