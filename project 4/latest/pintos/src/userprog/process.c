...
/* Start of Project 2 */
static bool load_segment_exec (struct file *file, off_t ofs, uint8_t *upage,
...
file_load (void *file_name_)
{
  ...
  if_.cs = SEL_UCSEG;
  if_.eflags = FLAG_IF | FLAG_MBS;
  success = is_file_exec (file_name, &if_.eip, &if_.esp);

  return success;
}

bool
is_file_exec (char *file_name, void (**eip) (void), void **esp UNUSED)
{
  struct Elf32_Ehdr ehdr;
  struct file *file = NULL;
  off_t file_ofs;
          ...
          if (validate_segment (&phdr, file))
            {
              ...
              if (!load_segment_exec (file, file_page, (void *) mem_page,
                                      read_bytes, zero_bytes, writable))
                goto done;
            }
          else
...
static bool
load_segment_exec (struct file *file, off_t ofs, uint8_t *upage,
