...
syscall_handler (struct intr_frame *f UNUSED) 
{
  ...
  switch (*syscall_num)
  {
    ...
    case SYS_OPEN: 
		   ...
		   /* Start of Project 4 */
		   if (inode_is_dir (fp->inode))
		   {
		     temp_fd->file = NULL;
		     temp_fd->dir = (struct dir *)fp;
		     temp_fd->is_dir = true;
		   }
		   else
		   {
    ...
    case SYS_READDIR:
		   ...
		   {
		     if (fd_name->is_dir)
		       f->eax = dir_readdir (fd_name->dir, file_name);
		     else
		       f->eax = 0;
		   }
		   break;
    ...
    case SYS_INUMBER:
                   ...
                   if (fd_name == NULL)
                     f->eax = -1;
		   else if (fd_name->is_dir)
		     f->eax = inode_get_sector (fd_name->dir->inode);
		   else
		     f->eax = inode_get_sector (fd_name->file->inode);
  }
